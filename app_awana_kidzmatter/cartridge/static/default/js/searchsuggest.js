	...
	if (app) {
		...
		app.searchsuggest = {
				...
				jQuery.getJSON(reqUrl, function(json) {
					...
					if(ansLength > 0) {
						...
						for(i=0; i < ansLength; i++) {
							...
							if (i==0) {newData += " style=\"border-top:none;\"";}
							newData += "><div class=\"suggestionterm\">" + json.suggestions[i].suggestion + "</div>";
			...
			repositionResultsDiv : function()
			{
				...
				app.searchsuggest.acResultsDiv.css("left", inLeft + 0); // to tweak
				app.searchsuggest.acResultsDiv.css("top", inTop + inHeight + 1);
				app.searchsuggest.acResultsDiv.css("width", inWidth + 10); // to tweak
