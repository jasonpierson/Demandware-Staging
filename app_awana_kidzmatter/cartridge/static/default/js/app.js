...
var app = (function(jQuery){
	...
	jQuery(document).bind('cbox_complete', function(){
        ...
        //app.initCorners("#cboxContent, .colorboxDialogHeader");
	});
	...
	return {
		...
		initCorners: function(container) {
			var settings = {
		        tl: { radius: 10 },
		        tr: { radius: 10 },
		        bl: { radius: 10 },
		...
		minicart: {
			...
			init: function() {
				...
				jQuery(".linkminicart").hover(function(e){(app.minicart.isShow() ? '': app.minicart.slide());});
			...
			// reset minicart
			reset: function() {
				jQuery(".linkminicart").unbind("hover");
			...
			slide: function() {
				...
				jQuery('.linkminicart').addClass('hovered');
			...
			close: function(delay) {
				if ( app.minicart.timer != null || delay == 0) {
					...
					jQuery('.linkminicart').removeClass('hovered');
				}
			},
		...
		quickView: {
			...
			show: function(options) {
			    ...
			    //make sure the dialog gets resized to fit content appropriately
			    jQuery('#QuickViewDialog').ajaxSuccess(function() {
			    	jQuery('#QuickViewDialog').dialog("option", "height", jQuery('#pdpMain').outerHeight()+30).css("height", "auto");
			    	jQuery('.quickview').css("height", "auto");
		...
		producttile : {
			...
			initAll: function() {
				...
				var quickViewOptions = {
					buttonSelector: "div.producttile div.productquickviewbtn",
					imageSelector: "div.producttile div.image",
					buttonLinkSelector: "div.producttile div.productquickviewbtn a"
				};
				...
				jQuery("div.producttile div.swatches > a").click(function(e) {
					if(jQuery(this).hasClass("sizechart") != true) {
			...
			selectVariation : function(swatch) {
				...
				var quickViewAnchor = swatch.parents(".producttile").find(".productquickviewbtn a");
