	...
	if (app) {
		...
		app.Product = function(response) {
			...
			var loadRecommendations = function(containerId) {
				if (jQuery(containerId+" .recommendations ul li").length > 0) {
					app.tooltip({id: containerId+" .recommendations ul li", options: {
			...
			var getAddToCartBtn = function(thisProduct) {
				var addToCartBtn = jQuery(thisProduct.containerId+" .addtocartbutton:last").click(function(e) {
					...
					if(jQuery(this).hasClass("addtocartbutton-disabled") == true)
						return;
					...
					//grab the registration number (for the church registration product(s) only)
					thisProduct.selectedOptions.RegistrationNumber = jQuery(thisProduct.containerId+" .registrationnumberinput:last").val();
			...
			var getQtyBox 		= function(thisProduct) {				
				...
				if (!isLoadingVar) {
					...
					var __availMsg = createAvMessage(thisProduct, thisProduct.selectedOptions.Quantity);
					setAvailabilityMsg(__availMsg);
					jQuery(thisProduct.containerId+" .addtocartbutton:last").attr("title", __availMsg);
				}
			}
			...
			var createAvMessage = function(thisProduct, val) {
				...
				if(val !== undefined && val != "NaN" && thisProduct.selectedVar != null)
				{
					...
					else if(val > thisProduct.selectedVar.avLevels.IN_STOCK && thisProduct.selectedVar.avLevels.BACKORDER > 0) {
						avMessage = jQuery.format(app.resources["REMAIN_BACKORDER"], thisProduct.selectedVar.avLevels.IN_STOCK);
					}
				}
				return avMessage;
			}
			...
			return {
				...
				enableA2CButton: function() {
					var __a2cBtn = jQuery(this.containerId+" .addtocart button:last");
					__a2cBtn.removeClass("addtocartbutton-disabled");
					__a2cBtn.addClass("addtocartbutton");
				},
				...
				disableA2CButton: function() {
					...
					var __a2cBtn = jQuery(this.containerId+" .addtocart button:last");
					__a2cBtn.removeClass("addtocartbutton");
					__a2cBtn.addClass("addtocartbutton-disabled");
				},
				...
				showSelectedVarAttrVal: function(varId, val, description) {
				
					jQuery(this.containerId+" .variationattributes div:not(.clear)").each(function(){
						var id = jQuery(this).data("data");
						
						if (varId === id) {
							jQuery(this).find('span.selectedvarval').html(description);
						}
					});
				},
				...
				*/
				varAttrSelected: function(e) {
					...
					this.showSelectedVarAttrVal(e.data.id, e.data.val || "", e.data.description || "");
				...
				* */
				...
				refreshView: function() {
					...
					if (!isLoadingVar && this.selectedVar != null) {
						// update availability
						if (thisProduct.getInStockDate() != "null") {
							jQuery("#ProductVarInStock").html("- "+jQuery.format(app.resources["IN_STOCK_DATE"], (new Date(thisProduct.getInStockDate())).toDateString() ));
						}
					...
					if (nonSelectedVars.length > 0) {
						...
						if(jQuery(thisProduct.containerId+" .addtocart button").hasClass("addtocartbutton") == true)
							jQuery(thisProduct.containerId+" .addtocartbutton:last").attr("title", availMsg);
						else
							jQuery(thisProduct.containerId+" .addtocartbutton-disabled:last").attr("title", availMsg);
					}					
				},
				/**
				...
				*/
				...
				show: function(options) {
					...
					if (model.master || model.variant) {
						...
						jQuery(thisProduct.containerId + " .variationattributes .swatches").each(function(){
							...
							var varEventHandler = function(e){
								...
								e.data = {id: pdpVarId, val: this.innerHTML, description: this.title};
								if (thisObj.parent().hasClass("unselectable")) {
									return false;
								}
								else if (thisObj.parent().hasClass("selected")) {
									// deselection
									e.data = {id: pdpVarId, val: null, description: this.title};
					...
					var __availMsg = app.resources[model.avStatus];
					setAvailabilityMsg(__availMsg);
					jQuery(thisProduct.containerId+" .addtocartbutton:last").attr("title", __availMsg);
