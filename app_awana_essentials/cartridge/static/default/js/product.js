						...
						if (model.bundle || model.productSet) {							
							...
							for(var i = 0; i < thisProduct.subProducts.length; i++) {
								...
								for(var j = 0; j < subProducts.length; j++) {
									if(subProducts[j].pid == thisProduct.subProducts[i].pid) {
										include = true;
									}
								}
								if(!include) {
									subProducts.push(thisProduct.subProducts[i]);
								}									
							}
					...
					jQuery.each(vals, function(){
						...
						if (this.val === selectedVal && this.images) {
							...
							if (this.images.small.length > 0) {
								...
								// Check to see if a quickview only image on the page has a source.  If so, change only the product image inside the QuickViewDialog.  If not, change the product image normally.
								if(jQuery(".productQuickviewTitle").attr("src")) {
									jQuery("#QuickViewDialog "+that.containerId+" .productimage").html("").append(jQuery("<img/>").attr("src", thisVal.images.large[0].url).attr("alt", thisVal.images.large[0].alt).attr("title", thisVal.images.large[0].title));
								} else {
				...
				* */
				...
				refreshView: function() {
					...
					if (!isLoadingVar && this.selectedVar != null) {
						...
						// update availability												jQuery(".ProductVarInStock").hide();
						if (thisProduct.getInStockDate() != "null" && jQuery("#ProductInStock").html() == "") {							/*							var output;							for (property in thisProduct.selectedVar) {							  output += property + ': ' + thisProduct.selectedVar[property]+'; ';							}							alert(output);							*/							// alert(thisProduct.selectedVar.id);																					
							jQuery("#ProductVarInStock"+thisProduct.selectedVar.id).html("- "+jQuery.format(app.resources["IN_STOCK_DATE"], (new Date(thisProduct.getInStockDate())).toDateString() ));														jQuery("#ProductVarInStock"+thisProduct.selectedVar.id).show();
						}
